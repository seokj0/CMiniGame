...
#define MAX_N 19
#define DRAW_BLACK printf("O")
#define DRAW_WHITE printf("X")
#define WHITE 2
#define BLACK 1
...
void inputCode(int* y, int* x) {
    ...
    *x = s[0] - 'A';
    ...
    printf("Input Coordinate (A19): \t\t\t");   //입력이 끝난 후 새로고침용 출력
...
int drawStone(int y, int x, int stone) { // stone이 1이면 흑, 2면 백 오류시 0 반환
    ...
    if (x < 0 || x >= MAX_N || y < 0 || y >= MAX_N || board[y][x]) // ||논리합(or),  board[x][y]에 값이 있으면 0(false)가 아니므로 True.
        return 0;
